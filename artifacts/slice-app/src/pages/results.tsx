...
export default function Results() {
  ...
  const isHost = localStorage.getItem(`slice_host_${code}`) !== null;
    ...
    <div className="min-h-[100dvh] bg-background p-4 md:p-8">
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-2 mt-4">
          ...
          <p className="text-muted-foreground text-lg">{results.merchantName || "Dinner"} • Total: ${results.totalBill.toFixed(2)}</p>
        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              {results.participants.map(p => (
                <div key={p.participantId} className="flex items-center justify-between pb-3 border-b last:border-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-secondary/10 text-secondary flex items-center justify-center shrink-0">
                      <User className="w-4 h-4" />
                    </div>
                    <span className="font-medium">{p.name} {p.isHost && <span className="text-xs text-muted-foreground ml-1">(Host)</span>}</span>
                  </div>
                  <span className="font-mono font-bold">${p.totalOwed.toFixed(2)}</span>
                </div>
              ))}
            </CardContent>
          </Card>
